...
def eval_queue(q, rank, out_dict, visa_pred_path, not_found_count):
    while not q.empty():
        vid_name, exp = q.get()  # 'OVIS/train/00e92ab4', '2'
        vid = exp_dict[vid_name]
        exp_name = f'{vid_name}_{exp}'

        try:
            # Check if vid_name exists
            if not os.path.exists(f'{visa_pred_path}/{vid_name}'):
                print(f'{vid_name} not found')
                with not_found_count.get_lock():  # Safely increment the counter
                    not_found_count.value += 1
                out_dict[exp_name] = [0, 0, 0, 0]
                continue

            # Proceed with evaluation if vid_name exists
...
if __name__ == '__main__':
    ...
    parser.add_argument("--visa_exp_path", type=str, default="/mnt/ali-sh-1/dataset/zeus/chenqr/datasets/qrchen_dataset/other-datasets/ReVOS/meta_expressions_valid_.json")
    parser.add_argument("--visa_mask_path", type=str, default="/mnt/ali-sh-1/dataset/zeus/chenqr/datasets/qrchen_dataset/other-datasets/ReVOS/mask_dict.json")
    parser.add_argument("--visa_foreground_mask_path", type=str, default="/mnt/ali-sh-1/dataset/zeus/chenqr/datasets/qrchen_dataset/other-datasets/ReVOS/mask_dict_foreground.json")
    ...
    # Counter for not found vid_names
    not_found_count = mp.Value('i', 0)
    ...
    if NUM_WORKERS > 1:
        ...
        for rank in range(NUM_WORKERS):
            p = mp.Process(target=eval_queue, args=(queue, rank, output_dict, args.visa_pred_path, not_found_count))
            p.start()
            processes.append(p)
        ...
        eval_queue(queue, 0, output_dict, args.visa_pred_path, not_found_count)

    # Print skipped vid_names
    print(f'Total skipped vid_names (not found): {not_found_count.value}')
