...
def main(args):
    ...
    transform = DirectResize(args.image_size) # ResizeLongestSide(args.image_size)

    model.eval()

    # ---------------------------- read data ------------------------------------
    meta_exp_path = "/mnt/ali-sh-1/dataset/zeus/chenqr/datasets/qrchen_dataset/other-datasets/ReVOS/meta_expressions_valid_.json"
    meta_exp = json.load(open(meta_exp_path, 'r'))
    video_folder = "/mnt/ali-sh-1/dataset/zeus/chenqr/datasets/qrchen_dataset/other-datasets/ReVOS/JPEGImages/"

    # ------------------ split into subsets for each GPU -------------------------
    vid_id_list = list(meta_exp['videos'].keys())
    vid_groups, total_jobs = {}, 0
    for vid_id in vid_id_list:
        vid_groups[vid_id] = []
        for exp_id in list(meta_exp['videos'][vid_id]['expressions'].keys()):
            vid_groups[vid_id].append((vid_id, exp_id))
            total_jobs += 1

    job_list_subset = []
    for vid_id, jobs in vid_groups.items():
        vid_hash = int(hashlib.md5(vid_id.encode()).hexdigest(), 16)
        if vid_hash % args.subset_num == args.subset_idx:
            job_list_subset.extend(jobs)

    total_infer = len(job_list_subset)
    print(f"ReVOS: Total videos: {len(vid_id_list)}, jobs: {total_jobs}")
    print(f"Subset {args.subset_idx} jobs: {len(job_list_subset)}")
    ...
    for vid_id, exp_id in job_list_subset:
        ...
        if os.path.exists(save_dir_vid_exp) and bool(os.listdir(save_dir_vid_exp)):
            continue
        ...
        color = random.choice([np.array([255, 0, 0]), np.array([0, 255, 0]), np.array([0, 0, 255])])
