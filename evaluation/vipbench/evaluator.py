...
parser = argparse.ArgumentParser(description="Evaluation of ViP-Bench results")
parser.add_argument('--result_path', type=str, default="")
parser.add_argument('--vipbench_split', type=str, default='human', choices=['human', 'bbox'], help="Split of ViP-Bench ('human' or 'bbox')")
args = parser.parse_args()

result_path = args.result_path  
vipbench_split = args.vipbench_split  

###### define the type of your visual prompt  ######
vipbench_path = "/mnt/ali-sh-1/usr/chenqirui/qrchen_dataset/ViP-Bench" #  "/path/to/vip-bench"
###### change your model name ######
model = 'model'

num_run = 5 # we set it as 5 in the paper
model_results_file = os.path.join(result_path, f"{vipbench_split}-answers.json")
...
grade_file = f'{vipbench_split}_{gpt_model}-grade-{num_run}runs.json'
grade_file = os.path.join(result_path, grade_file)

# score results regarding capabilities/capability integration to save
cap_score_file = f'{vipbench_split}_{sub_set_name}{gpt_model}-cap-score-{num_run}runs.csv'
cap_score_file = os.path.join(result_path, cap_score_file)
cap_int_score_file = f'{vipbench_split}_{sub_set_name}{gpt_model}-cap-int-score-{num_run}runs.csv'
