...
def main(args):
    ...
    # ---------------------------- read data ------------------------------------
    meta_exp_path = "/mnt/ali-sh-1/dataset/zeus/chenqr/datasets/qrchen_dataset/other-datasets/ReasonVOS/meta_expressions.json"
    meta_exp = json.load(open(meta_exp_path, 'r'))["videos"]
    video_folder = "/mnt/ali-sh-1/dataset/zeus/chenqr/datasets/qrchen_dataset/other-datasets/ReasonVOS/JPEGImages"
    ...
    print(f"Total {len(list(meta_exp.keys()))} videos, {len(job_list)} jobs.")

    # ------------------ start processing -------------------------
    for vid_key, exp_id, exp_text, is_sent in job_list_subset:
        # prepare folder
        save_dir_vid = os.path.join(args.vis_save_path, vid_key)
        os.makedirs(save_dir_vid, exist_ok=True)

        save_dir_vid_exp = os.path.join(args.vis_save_path, vid_key, exp_id)
