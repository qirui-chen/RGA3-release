...
def parse_args(args):
    ...
    parser.add_argument("--split", default="valid_u", type=str, choices=['valid_u', 'valid'])
...
def main(args):
    ...
    # ---------------------------- read data ------------------------------------
    meta_exp_path = f"/mnt/ali-sh-1/dataset/zeus/chenqr/datasets/qrchen_dataset/other-datasets/mevis/{args.split}/meta_expressions.json"
    meta_exp = json.load(open(meta_exp_path, 'r'))
    video_folder = f"/mnt/ali-sh-1/dataset/zeus/chenqr/datasets/qrchen_dataset/other-datasets/mevis/{args.split}/JPEGImages"
    ...
    print(f"Total {len(vid_id_list)} videos, {len(job_list)} jobs.")
    # ------------------ start processing -------------------------
