...
class UniGRModel(Qwen2_5_VLForConditionalGeneration):
    ...
    def initialize_sam_modules(self, config):
        # SAM
        self.sam_model = build_sam_vit_h(config.sam_pretrained)
    ...
    def model_forward(
        ...
        **kwargs,
        ...
        batch_size = images_sam.shape[0]
        assert batch_size == len(offset) - 1

        images_reshape = rearrange(images_sam, 'b t c h w -> (b t) c h w')
        image_embeddings_reshape = self.get_visual_embs(images_reshape)     # [(b t) c h w]
        image_embeddings = rearrange(image_embeddings_reshape, '(b t) c h w -> b t c h w', b=batch_size)
        ...
        for i in range(len(pred_embeddings)):
            ...
            for frame_idx in range(image_embeddings.shape[1]):
                low_res_masks, iou_predictions = self.sam_model.mask_decoder(
                    image_embeddings=image_embeddings[i, frame_idx].unsqueeze(0),
                ...
                pred_masks_cur_vid.append(pred_mask[:, 0])
            pred_masks_cur_vid = torch.cat(pred_masks_cur_vid, dim=0)
    ...
    def evaluate(
        ...
        with torch.no_grad():
            ...
            num_frames_sam = images_sam.shape[1]
            ...
            for i in range(len(pred_embeddings)):
                ...
                pred_masks_cur_vid = []
                for frame_idx in range(num_frames_sam):
                    image_embeddings = self.get_visual_embs(images_sam[:, frame_idx])
                    low_res_masks, iou_predictions = self.sam_model.mask_decoder(
