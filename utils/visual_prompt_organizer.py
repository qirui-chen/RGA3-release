...
import re 
import json
from .visual_prompt_generator import image_blending, color_pool, words_shape
...
def vip_conv_generator(source, sampled_shapes, dataset_type, sub_type = ''):
    ...
    if dataset_type == 'refcocog':
        ...
        if sub_type == 'gpt4v':
            ...
            convs_source.append([prompt, source['answer']])
    elif dataset_type == 'osprey':
        for bbox_index, ( color_name, _ , predefined_shape) in enumerate( sampled_shapes ):
            _, word2 = words_shape[predefined_shape]
            text ='the '
            if color_name is not None:
                text += color_name + ' '
            text += word2
            for i in range(len(source['conversations'])):
                source['conversations'][i]['value'] = re.sub(fr'<reg(in|ion){bbox_index+1}?>', text, source['conversations'][i]['value'])
        return source['conversations']
    conv = [] 
...
def vip_processor(source, image, image_size_anchor, data_args):
    ...
    if dataset_type in {'vg_rel', 'v7w', 'pointQA_twice', 'osprey'}:
    ...
    if dataset_type in {'vcr'}:
        ...
        for instance_idx, (bbox, segmentation) in enumerate(zip(source['bboxes'], source['segmentations'])):
            ...
            while color_rgb in used_colors and num_retry < 10:
                num_retry += 1
                color_name, color_rgb = random.choice(list(color_pool.items()))
    ...
    for instance_idx, (bbox, segmentation) in enumerate(zip(source['bboxes'], source['segmentations'])):
        ...
        image, _ = image_blending(image,  shape = sampled_shape, image_size_anchor = image_size_anchor, rgb_value=color_rgb, bbox_coord= bbox, segmentation=segmentation, visual_prompt_style = visual_prompt_style, alpha = alpha)
